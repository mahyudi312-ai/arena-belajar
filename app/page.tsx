  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-6">
      ...
      <div className="grid md:grid-cols-3 gap-6 mt-16 max-w-5xl">
        {[
          { emoji: "📚", title: "Materi Lengkap", desc: "8 mapel, kelas 1-6 SD" },
          { emoji: "🎮", title: "Game Seru", desc: "Tebak gambar, memory, cari kata" },
          { emoji: "🏆", title: "Jadi Juara", desc: "Kumpulkan kristal & medali" }
        ].map((f, i) => (
          <motion.div key={i} initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 0.3 + i * 0.2 }} className="card-arena text-center">
            <div className="text-6xl mb-3">{f.emoji}</div>
